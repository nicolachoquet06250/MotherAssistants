...
let setup_default = (role, after_init = null) =>
	$(document).ready(() => {
		...
		function onNotificationClick(data, type) {
			...
			else if (data.notification_type === 'medias') {
				selector = '#photos_in_messages';
			}
			let instance = M.Modal.getInstance(document.querySelector(selector));
			instance.open();
			if(type === 'toast') {
				console.log(`j'ai cliqué sur un toast`);
			}
...
let setup_account = role => setup_default(role, role => {
	...
	let ma_setup_account = () => {
		default_setup_account();
	};
	let parent_setup_account = () => {
		let profile_pic_resize = () => document.querySelector('.profile-pic').style.height = document.querySelector('.profile-pic').offsetWidth + 'px';
		let get_role_in_french = role => {
			let roles = {
				mother: 'Maman',
				father: 'Papa',
			...
			return role in roles ? roles[role] : null;
		};
		profile_pic_resize();
		window.onresize = () => {
			profile_pic_resize();
		};
		document.querySelector('#parent_role').innerHTML = get_role_in_french(document.querySelector('#parent_role').getAttribute('data-role'));
	};
	switch (role) {
		case 'ma':
			ma_setup_account();
...
<div class="row">
	...
	let ma_setup_children = () => {
		default_setup_children();
	};
	let parent_setup_children = () => {
		default_setup_children();
		let birth_day = document.querySelector('#birth_day').getAttribute('data-date');
		let age = calculate_age(birth_day);
		birth_day = birth_day.split('-');
		birth_day = `${birth_day[2]}/${birth_day[1]}/${birth_day[0]}`;
		document.querySelector('#birth_day').innerHTML = birth_day;
		document.querySelector('#age').innerHTML = `${age.age} ${age.type === 'm' ? 'mois' : `an${age.age > 1 ? 's' : ''}`}`;
		let resize_buttons_child = () => document.querySelectorAll('.button-child').forEach(button => {
			button.style.height = button.offsetWidth + 'px';
			button.querySelector('.row').style.marginTop = (button.offsetHeight - button.querySelector('.row').offsetHeight) / 2 + 'px';
		});
		resize_buttons_child();
		window.onresize = () => resize_buttons_child();
	};
